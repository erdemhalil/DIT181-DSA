...
public class ScapegoatTree<Key extends Comparable<Key>, Value> {
    ...
    private Node put(Node node, Key key, Value val) {
        ...
        //Update node's size depending on its children
        node.size = 1 + size(node.left) + size(node.right);
        //Update node's size depending on its child with greater height than the other one
        node.height = 1 + Math.max(height(node.left), height(node.right));

        //If the invariant does not hold, rebuild the subtree rooted at this node
    ...
    private Node balanceNodes(ArrayList<Node> nodes, int lo, int hi) {
        ...
        Node node = new Node(nodes.get(mid).key, nodes.get(mid).val);

        node.left = balanceNodes(nodes, lo, mid - 1);
        node.right = balanceNodes(nodes, mid + 1 , hi);

        //Set node's size depending on its children
        node.size = 1 + size(node.left) + size(node.right);
        //Set node's size depending on its child with greater height
        node.height = 1 + Math.max(height(node.left), height(node.right));
