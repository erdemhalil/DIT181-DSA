...
public class Lab3 {
    ...
    // Phase 2: Build index of n-grams.
      ...
      // TODO: build index of n-grams
        ...
        for(Path path : files.keys()){                     //O(D)
            //Go through all the ngrams in each file
            for(Ngram ngram : files.get(path)){            //O(K)  //Nested -> O(D * K) = O(N)
                /*
                * If there's no node in the tree that matches the Key Ngram, create a new Node in the tree
                * where Ngram is the Key and the file that contains the Ngram is the Value
                *
                * Otherwise, if there's a node that matches the Key Ngram, we just update that particular node's
                * associated Value by adding a new element (filename) to the already existing ArrayList
                * */
                if(!index.contains(ngram)) {                    //O(log N)
                    ArrayList<Path> paths = new ArrayList<>(); //O(1)
                    paths.add(path);                          //O(1)
                    index.put(ngram, paths);                 //O(log N)
                } else {
                    index.get(ngram).add(path);            //O(log N)
                }                                         //TOTAL: O(N Log N)
            }
        }
        return index;
    ...
    static ScapegoatTree<PathPair, Integer> findSimilarity(ScapegoatTree<Path, Ngram[]> files, ScapegoatTree<Ngram, ArrayList<Path>> index) {
        ...
        for(Ngram ngram : index.keys()){      //O(N)
            for(int i = 0; i < index.get(ngram).size(); i++){ //O(1)  the number of 5-grams that occur in more than one file is a small constant.
                for(int j = i + 1; j < index.get(ngram).size(); j++){ //O(1) -- same reason
                    PathPair pair = new PathPair(index.get(ngram).get(i), index.get(ngram).get(j));
                    if (!similarity.contains(pair)) { //O(Log N)
                        similarity.put(pair, 1); //O(Log N)
                    } else {
                        similarity.put(pair, similarity.get(pair) + 1);  //O (Log N)
                    }                                                       //TOTAL: O(N Log N)
