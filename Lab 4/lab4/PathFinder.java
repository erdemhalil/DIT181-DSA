...
public class PathFinder<Node> {
    ...
    public Result searchUCS(Node start, Node goal) {
        ...
        PQEntry entry = new PQEntry(start, 0,0, null);
        pqueue.add(entry);
        while(!pqueue.isEmpty()){
            ...
            if(!visitedNodes.contains(entry.node)) {
                ...
                for (DirectedEdge<Node> edge : graph.outgoingEdges(entry.node)) {
                    ...
                    PQEntry newEntry = new PQEntry(edge.to(), costToNext,0, entry);
                    pqueue.add(newEntry);
                }
            }
        }
    ...
    public Result searchAstar(Node start, Node goal) {
         ...
         * Change below this comment  *
         ******************************/
        Queue<PQEntry> pqueue = new PriorityQueue<>(Comparator.comparingDouble(entry -> entry.estimatedCost + entry.costToHere));
        Set<Node> visitedNodes = new HashSet<>();
        PQEntry entry = new PQEntry(start, 0, graph.guessCost(start, goal), null);
        pqueue.add(entry);
        while(!pqueue.isEmpty()){
            entry = pqueue.remove();
            iterations++;
            if(!visitedNodes.contains(entry.node)) {
                ...
                for (DirectedEdge<Node> edge : graph.outgoingEdges(entry.node)) {
                    double costToNext = entry.costToHere + edge.weight();
                    entry.estimatedCost = graph.guessCost(edge.to(), goal);
                    PQEntry newEntry = new PQEntry(edge.to(), costToNext, entry.estimatedCost, entry);
                    pqueue.add(newEntry);
                }
            }
        }
    ...
    private class PQEntry {
        ...
        PQEntry(Node n, double c, double e, PQEntry bp) {
            node = n;
            costToHere = c;
            estimatedCost = e;
            backPointer = bp;
